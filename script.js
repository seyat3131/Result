...
/* ---------------------------------------------------- */
/* RENK SÜTUNU GİZLEME VE GENİŞLİK AYARLARI (NORMAL MOD) */
/* ---------------------------------------------------- */

/* NORMAL MODDA (table-header VE score-row) Genişlikler (Toplam %100) */
/* RANK (15%) + TEAM (50%) + TOTAL (35%) = 100% */
.rank-col { flex-basis: 15%; max-width: 15%; } 
.total-col { flex-basis: 35%; max-width: 35%; } 
.team-col { flex-basis: 50%; max-width: 50%; text-align: left; padding-left: 15px; justify-content: flex-start; } 

/* Renk Sütununu (color-col) Normal Modda Başlıklarda ve Satırlarda Gizle */
.color-col {
    display: none; 
    flex-basis: 0;
    max-width: 0;
...
.table-header.admin-mode-active .color-col {
    display: flex; /* YÖNETİCİ GİRİŞİ: Görünür yap */
    flex-basis: 15%; /* Yönetici için yer aç */
    max-width: 15%;
    padding: 10px 5px;
    border-right: 1px solid #333333;
}
...
/* RANK (15) + TEAM (40) + COLOR (15) + TOTAL (30) = 100% */
.score-row.admin-mode-active .rank-col { flex-basis: 15%; max-width: 15%; } 
.score-row.admin-mode-active .total-col { flex-basis: 30%; max-width: 30%; } 
.score-row.admin-mode-active .team-col { flex-basis: 40%; max-width: 40%; } 
